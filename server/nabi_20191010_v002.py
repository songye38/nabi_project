...
def offWrite():
	...
	curs = mysql.cursor()
	try:
		sql = "INSERT INTO `onActionUser` (`onUser_user_no`, `onUser_on_no`, `onUser_comm_no`, `onUser_content`,`onUser_date`) VALUES (%s, %s,%s,%s,now())"
...
@app.route('/off/update',methods=['POST'])
def offUpdate():
	data = request.json
	curs = mysql.cursor()
	try:
		sql = "UPDATE onActionUser SET onUser_content=%s, onUser_date= now() WHERE onUser_no= %s"
		print(sql)
