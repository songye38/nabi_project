...
@app.route('/searchtabJoinStatus/<_comId>/<_userId>',methods = ['GET'])
def joinStatusSearchtab(_comId,_userId):
    result_len = db.comm.count_documents({ "$and": [{"_id": ObjectId(_comId)},{"userList.user_id": ObjectId(_userId)}]})
    # results= db.comm.aggregate([
    #     { "$match": {"comm_id" : ObjectId(_comId)}},
    #     { "$unwind": "$userList" },
    #     { "$match": {"userList.user_id": ObjectId(_userId)}},
