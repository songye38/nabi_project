...
@app.route('/talk/like/<_talkNo>/<_talkUser_no>/<_userNo>',methods=['GET'])
def talkLike(_talkNo,_talkUser_no,_userNo):
	curs = mysql.cursor()
	first_search_query = 'select * from talkHeart where talkHeart_talkUser_no=%s and talkHeart_talk_no=%s and talkHeart_user_no=%s'
	curs.execute(first_search_query,(_talkUser_no,_talkNo,_userNo))
	#처음 좋아요를 누르는 경우 -> 처음 좋아요를 누른 경우 아직 데이터베이스에 저장되어있지 않기 때문에 fetchall의 결과가 0임
	if(len(curs.fetchall()))==0:
		try:
			insert_query = "INSERT INTO `talkHeart` (`talkHeart_talkUser_no`, `talkHeart_user_no`, `talkHeart_talk_no`,`talkHeart_status`,`talkHeart_date`) VALUES (%s,%s,%s,%s,now())"
			curs.execute(insert_query,(_talkUser_no,_userNo,_talkNo,"1"))
			mysql.commit()
		except (MySQLdb.Error, MySQLdb.Warning) as e:
			print(e)
		finally:
	...
	#한번 하트를 눌러서 저장을 한 경우 fetchall의 결과가 0이 아님 
		...
		search_query = "select talkHeart_status from talkHeart where talkHeart_talkUser_no=%s and talkHeart_user_no=%s and talkHeart_talk_no=%s "
		curs.execute(search_query,(_talkUser_no,_userNo,_talkNo))
		search_result = curs.fetchall()
		try :
			update_query = "UPDATE talkHeart SET talkHeart_status = %s ,talkHeart_date =now() WHERE talkHeart_talkUser_no=%s and talkHeart_user_no=%s and talkHeart_talk_no=%s"
			status = "0" if search_result[0][0]=="1" else "1"
			curs.execute(update_query,(status,_talkUser_no,_userNo,_talkNo))
			mysql.commit()
		except (MySQLdb.Error, MySQLdb.Warning) as e:
			print(e)
		finally:
