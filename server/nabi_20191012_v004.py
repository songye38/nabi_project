...
def comOnGoing(_comNo,_order):
	curs = mysql.cursor()
	off_search_query = "select * from offAction where off_status = 1 and off_comm_no=%s"
	curs.execute(off_search_query,(_comNo))
	off_result = curs.fetchall()
	return { "result": "success", "value": off_result,"message":"성공적으로 조회하였습니다."}
