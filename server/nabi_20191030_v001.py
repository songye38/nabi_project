...
#===============================================================================================mysql 연결
client = MongoClient(host= '13.209.35.234', port =54825)
db = client.nabiDB
#===============================================================================================회원가입
 #회원가입 관련 API part   
...
def comShow(_keyword):
    if _keyword =='all':
        ...
        results = db.comm.find({ "$or": [{"title": "/_keyword/" }, {"category": "/_keyword/" }]})
        json_docs = []
        for doc in results:
            json_doc = json.dumps(doc, default=json_util.default)
            o = json.loads(json_doc)
...
def comJoin():
    data = request.json
    result_len = db.comm.count_documents({"userList.user_id": ObjectId(data['user_id'])})
    if result_len !=0:
        return { "result": "fail", "value": "fail","message":"이미 가입되어 있습니다."}
    else :
        db.comm.update({"_id" : ObjectId(data['comm_id'])},{"$push":{"userList":{"user_id":ObjectId(data['user_id']),"date": datetime.datetime.utcnow(),}}})
        db.point.update({"user_id" : ObjectId(data['user_id'])},{"$push":{"pointList":{"comm_id" : ObjectId(data['comm_id']),"comm_name" : data['comm_name'],"comm_pic" : data['comm_pic'],"action_id" : data['comm_id'],"type" : 'join_comm',"date" : datetime.datetime.utcnow()}}})
        db.point.update({"user_id" : ObjectId(data['user_id'])},{ "$inc": { "score": +10}})
...
def comJoinDel():
    ...
    result_len = db.comm.count_documents({"userList.user_id": ObjectId(data['user_id'])})
    if result_len ==0:
        return {"result":"fail","value":"fail","message":'가입되어 있지 않습니다.'}
    else:
        db.comm.update({ "_id": ObjectId(data['comm_id'])},{ "$pull": { "userList": { "user_id": ObjectId(data['user_id'])}}});
        db.point.update({"user_id" : ObjectId(data['user_id'])},{"$push":{"pointList":{"comm_id" : ObjectId(data['comm_id']),"comm_name" : data['comm_name'],"comm_pic" : data['comm_pic'],"action_id" : data['comm_id'],"type" : 'del_comm',"date" : datetime.datetime.utcnow()}}})
        db.point.update({"user_id" : ObjectId(data['user_id'])},{ "$inc": { "score": -10}})
...
#개별 비대위 페이지 API part
#진행중인 행동들, 대화 보여주기
@app.route('/<_comId>/onGoing/<_order>',methods=['GET'])
def comOnGoing(_comId,_order):	
...
@app.route('/<_comId>/participate/<_userId>/<_order>',methods=['GET'])
def comParticipate(_comId,_order,_userId):
    offAction_results = db.offAction.find({ "$and": [{"comm_id": ObjectId(_comId)}, {"status": 0 },{ "userList.user_id": ObjectId(_userId)}]})
    onAction_results = db.onAction.find({ "$and": [{"comm_id": ObjectId(_comId)}, {"status": 0 },{ "userList.user_id" : ObjectId(_userId)}]})
    talk_results = db.talk.find({ "$and": [{"comm_id": ObjectId(_comId)}, {"status": 0 },{ "userList.user_id": ObjectId(_userId)}]})
    share_results = db.share.find({ "$and": [{"comm_id": ObjectId(_comId)}, {"status": 0 },{ "userList.user_id": ObjectId(_userId)}]})
    json_docs = []
    for doc in offAction_results:
        json_doc = json.dumps(doc, default=json_util.default)
        o = json.loads(json_doc)
...
@app.route('/<_comId>/archive/<_userId>/<_order>',methods=['GET'])
def comArchive(_comId,_userId,_order):
...
def offWrite():
    ...
    if result_len !=0 :
        ...
        db.offAction.update({"_id" : ObjectId(data['offaction_id'])},{"$push":{"userList":{
			 "user_id":ObjectId(data['user_id']),
			 "date": datetime.datetime.utcnow(),
		   }}})
        db.point.update({"user_id" : ObjectId(data['user_id'])},{"$push":{"pointList":{"comm_id" : ObjectId(data['comm_id']),"comm_name" : data['comm_name'],"comm_pic" : data['comm_pic'],"action_id" : data['offaction_id'],"type" : 'join_off',"date" : datetime.datetime.utcnow()}}})
        db.point.update({"user_id" : ObjectId(data['user_id'])},{ "$inc": { "score": +5}})
        return { "result": "success", "value": data,"message":"성공적으로 데이터베이스에 저장하였습니다."}

@app.route('/off/update',methods=['PUT'])
def offUpdate():
    data = request.json
    db.offAction.update({ "_id": ObjectId(data["offaction_id"])},{ "$pull": { "userList": {"user_id": ObjectId(data["user_id"])}}});
    db.point.update({"user_id" : ObjectId(data['user_id'])},{"$push":{"pointList":{"comm_id" : ObjectId(data['comm_id']),"comm_name" : data['comm_name'],"comm_pic" : data['comm_pic'],"action_id" : data['offaction_id'],"type" : 'del_off',"date" : datetime.datetime.utcnow()}}})
    db.point.update({"user_id" : ObjectId(data['user_id'])},{ "$inc": { "score": -5}})
...
def talkRead(_comId,_id):
    results = db.talk.find({"comm_id": ObjectId(_comId),"_id":ObjectId(_id)})
    json_docs = []
    for doc in results:
        json_doc = json.dumps(doc, default=json_util.default)
        o = json.loads(json_doc)
...
def talkWrite():
    ...
    db.talk.update({"_id" : ObjectId(data['talk_id'])},{"$push":{"userList":{
			 "user_id":ObjectId(data['user_id']),
			 "content" : data['content'],
			 "date": datetime.datetime.utcnow(),
		   }}})
    db.point.update({"user_id" : ObjectId(data['user_id'])},{"$push":{"pointList":{"comm_id" : ObjectId(data['comm_id']),"comm_name" : data['comm_name'],"comm_pic" : data['comm_pic'],"action_id" : data['talk_id'],"type" : 'join_talk',"date" : datetime.datetime.utcnow()}}})
    db.point.update({"user_id" : ObjectId(data['user_id'])},{ "$inc": { "score": +3}})
    return { "result": "success", "value": data,"message":"성공적으로 데이터베이스에 저장하였습니다."}
...
def talkUpdate():
    ...
    if data['content']=='':
        return {"result":"fail","value":data,"message":"내용이 입력되지 않았습니다."}
    db.talk.update({"_id":ObjectId(data['talk_id']),"userList.user_id": ObjectId(data['user_id'])}, { "$set": {"userList.$.content": data['content'],"userList.$.date":datetime.datetime.utcnow()}})
    db.point.update({"user_id" : ObjectId(data['user_id'])},{"$push":{"pointList":{"comm_id" : ObjectId(data['comm_id']),"comm_name" : data['comm_name'],"comm_pic" : data['comm_pic'],"action_id" : data['talk_id'],"type" : 'edit_talk',"date" : datetime.datetime.utcnow()}}})
    return {"result":"success","value":data,"message":"내용이 정상적으로 수정되었습니다."}

@app.route('/talk/delete',methods=['DELETE'])
def talkDelete():
    data = request.json
    db.talk.update({ "_id": ObjectId(data['talk_id'])},{ "$pull": { "userList": { "user_id": ObjectId(data['user_id'])}}});
    db.point.update({"user_id" : ObjectId(data['user_id'])},{"$push":{"pointList":{"comm_id" : ObjectId(data['comm_id']),"comm_name" : data['comm_name'],"comm_pic" : data['comm_pic'],"action_id" : data['talk_id'],"type" : 'del_talk',"date" : datetime.datetime.utcnow()}}})
    db.point.update({"user_id" : ObjectId(data['user_id'])},{ "$inc": { "score": -3}})
...
def talkLike(_talkId,_talkUser_Id,_userId):
    ...
    if result_len ==0:
        ...
        db.heart.update({"user_id" : ObjectId(_userId)},{"$push":{"heartList":{"action_id" : ObjectId(_talkId),"target_user_id":ObjectId(_talkUser_Id),"status" : 1,"date" :datetime.datetime.utcnow()}}})
        return {"result":"success","value":data,"message":"하트가 등록되었습니다."}
...
def shareWrite():
    ...
    if data['content']=='':
        return {"result":"fail","value":data,"message":"내용이 입력되지 않았습니다."}	
    db.share.update({"_id" : ObjectId(data['share_id'])},{"$push":{"userList":{
			 "user_id":ObjectId(data['user_id']),
			 "content" : data['content'],
		     "img1" : data['img1'],
			 "img2" : data['img2'],
    ...
    db.point.update({"user_id" : ObjectId(data['user_id'])},{"$push":{"pointList":{"comm_id" : ObjectId(data['comm_id']),"comm_name" : data['comm_name'],"comm_pic" : data['comm_pic'],"action_id" : data['share_id'],"type" : 'join_share',"date" : datetime.datetime.utcnow()}}})
    db.point.update({"user_id" : ObjectId(data['user_id'])},{ "$inc": { "score": +3}})
    return { "result": "success", "value": data,"message":"성공적으로 데이터베이스에 저장하였습니다."}
...
def shareUpdate():
    ...
    if data['content']=='':
        return {"result":"fail","value":data,"message":"내용이 입력되지 않았습니다."}
    db.share.update({"_id":ObjectId(data['share_id']),"userList.user_id": ObjectId(data['user_id'])}, { "$set": {
    ...
    db.point.update({"user_id" : ObjectId(data['user_id'])},{"$push":{"pointList":{"comm_id" : ObjectId(data['comm_id']),"comm_name" : data['comm_name'],"comm_pic" : data['comm_pic'],"action_id" : data['share_id'],"type" : 'edit_share',"date" : datetime.datetime.utcnow()}}})
    return {"result":"success","value":data,"message":"내용이 정상적으로 수정되었습니다."}

@app.route('/share/delete',methods=['DELETE'])
def shareDelete():
    data = request.json
    db.share.update({ "_id": ObjectId(data['share_id'])},{ "$pull": { "userList": { "user_id": ObjectId(data['user_id'])}}});
    db.point.update({"user_id" : ObjectId(data['user_id'])},{"$push":{"pointList":{"comm_id" : ObjectId(data['comm_id']),"comm_name" : data['comm_name'],"comm_pic" : data['comm_pic'],"action_id" : data['share_id'],"type" : 'del_share',"date" : datetime.datetime.utcnow()}}})
    db.point.update({"user_id" : ObjectId(data['user_id'])},{ "$inc": { "score": -3}})
    return {"result":"success","value":data,"message":"내용이 정상적으로 삭제되었습니다.."}
...
def changePic():
    data = request.json
    db.user.update({"_id":ObjectId(data['user_id'])},{"$set":{"pic": data['pic']}})
