...
import CommListModal from '../../customComponents/CommListModal';
import ActTypeModal from '../../customComponents/ActTypeModal';
...
export default class ArchiveScreen extends Component{
        ...
        Promise.all([fetch(url1), fetch(url2),fetch(commName)])
          ...
          .then(([res1, res2,res3]) => {
            this.setState({
                ...
                commNameList : res3.value,
            });
            if(this.state.commNameList!=undefined){
                this.setState({
                    isLoading : false,
                })
            }
    ...
    setCommInfo = (commId,commName) => {
        this.setState({commId: commId,commName : commName}, function() {
            this.componentDidMount();
    });
    }
    setActType = (type,typeTitle)=>{
        this.setState({type: type,typeTitle : typeTitle}, function() {
            this.componentDidMount();
        });
    }
    setDate = (selectedDate)=>{
        if(selectedDate=='all'){
            this.setState({date: selectedDate,dateText :'모든 기간'}, function() {
            this.componentDidMount();
        });
        }else{
            this.setState({date: selectedDate,dateText :selectedDate }, function() {
            this.componentDidMount();
        });
        }
    }
            ...
            <View style={styles.container}>
                <View style={styles.categorySection}>
                    <View style = {styles.cagegoryItem}>
                        <Text style={styles.categoryTitle} numberOfLines = {1}>{this.state.commName}</Text>
                        <CommListModal title = "비대위 목록" type = "comm" dataset = {this.state.commNameList} parentCallback = {this.setCommInfo} underlayColor='white'></CommListModal>
                    </View>
                    <View style = {styles.cagegoryItem}>
                        <Text style={styles.categoryTitle}>{this.state.typeTitle}</Text>
                        <ActTypeModal title = "활동 종류" type = "act" parentCallback = {this.setActType} underlayColor='white'></ActTypeModal>
                    </View>
                    <View style = {styles.cagegoryItem}>
                        <Text style={styles.categoryTitle}>{this.state.dateText}</Text>
                        <MyDatePicker parentCallback = {this.setDate}></MyDatePicker>
                    </View>
                </View>
                                        ...
                                        <View style = {styles.listContentSection} key={index}>
                                            ...
                                            <View style={styles.listMainContent}>
                                               <View style = {styles.title}>
                                                    <Text style={styles.dateText} numberOfLines = {1}>{element.pointList.comm_name}  |  {this.fotmattingDate(element.pointList.date.$date)}</Text>
                                                </View>  
